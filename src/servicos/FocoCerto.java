...
public class FocoCerto extends ModuloTelemedicina {
    ...
    public void executar(Scanner scanner) {
        ...
        if (respostaEnquadramento.equalsIgnoreCase("Sim")) {
            ...
            System.out.println("Caio: Entendi, parece que seu rosto não está bem enquadrado. Por favor, ajuste a posição da câmera para centralizar melhor seu rosto.");
        }
        ...
        if (respostaIluminacao.equalsIgnoreCase("Sim")) {
            ...
            System.out.println("Caio: Tente se posicionar em um local com mais luz ou ajustar a iluminação para melhorar a imagem.");
        }

        if (simCount == 3) {
            System.out.println("Caio: Apesar de você ter respondido 'Sim' para tudo, às vezes mesmo quando parece certo pode haver pequenos ajustes.");
            System.out.println("      -> Verifique se o seu rosto está realmente centralizado, sem cortes ou distorções.");
        ...
        boolean cameraOk = respostaEnquadramento.equalsIgnoreCase("Sim") && respostaCentralizado.equalsIgnoreCase("Sim");
        boolean microfoneOk = true; // pode ser validado em outro ponto
        if (validarEquipamentos(cameraOk, microfoneOk)) {
            System.out.println("Caio: Todos os equipamentos estão funcionando corretamente.");
        } else {
            System.out.println("Caio: Há algum problema com os equipamentos. Verifique antes de iniciar a teleconsulta.");
        }
    }

    /**
     * Método para validar se os equipamentos essenciais estão funcionando.
     * @param camera indica se a câmera está funcionando e bem posicionada
     * @param microfone indica se o microfone está funcionando
     * @return true se ambos estiverem OK, false caso contrário
     */
    public boolean validarEquipamentos(boolean camera, boolean microfone) {
        return camera && microfone;
    }
}
